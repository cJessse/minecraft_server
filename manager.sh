...
# Valores padrão caso não definidos no config.env
WORKSPACE_DIR="${SERVER_ROOT_DIR:-$SCRIPT_DIR}"
    ...
    if [ "${ENABLE_PLAYIT:-true}" = "true" ]; then
        ...
        if ! pgrep -f "playitd" > /dev/null; then
            if command -v playitd &> /dev/null; then
                ...
                echo -e "${RED}playitd não encontrado! Execute ./setup.sh primeiro.${NC}"
            fi
        else
    ...
    if [ "$SERVER_MODE" = "crafty" ]; then
        ...
        if ! pgrep -f "python3 main.py" > /dev/null; then
            if [ -d "$CRAFTY_DIR/.venv" ] && [ -d "$CRAFTY_DIR/crafty-4" ]; then
                ...
                echo -e "${RED}Ambiente do Crafty não encontrado. Execute ./setup.sh!${NC}"
            fi
        else
            echo -e "${YELLOW}Já em execução${NC}"
        fi
...
backup_world() {
    ...
    if [ -d "$WORKSPACE_DIR/minecraft/crafty/crafty-4/servers" ]; then
        # Localiza primeiro servidor configurado no Crafty
