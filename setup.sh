...
if [ ! -f "config.env" ]; then
    echo -e "${YELLOW}[1/5] Criando config.env a partir do template...${NC}"
    cp config.env.example config.env
    echo -e "${GREEN}✓ config.env criado!${NC}"
else
    echo -e "${GREEN}✓ config.env já existente.${NC}"
fi
...
echo -e "\n${YELLOW}[5/5] Ajustando permissões dos scripts...${NC}"
chmod +x manager.sh setup.sh 2>/dev/null || true
